...
void main()
{
    ...
    vec4 color = texelFetch(prevpass, ivec2(gl_FragCoord.x, gl_FragCoord.y), 0);
    
    float ao = smartDeNoise(aobuffer, texcoord, uSigma, uKSigma, uThreshold).r;
        
    // Calculate luminance of the scene color
    float luminance = dot(color.rgb, vec3(0.299, 0.587, 0.114));

    // Adjust the SSAO effect based on luminance
    float i = mix(0.05, 0.2, 1.0 - luminance); // Stronger effect in darker areas

    color.r = max(color.r - (1.0f - ao) * i, 0.0f);
    color.g = max(color.g - (1.0f - ao) * i, 0.0f);
    color.b = max(color.b - (1.0f - ao) * i, 0.0f);
    outColor = color;
}