local function GameMenuButtonCallback(Event, Extra)
    if (KEY_ESCAPE == Event.data and Extra) then
        ...
        if (game.fpsPlayer ~= nil) then
            ...
            game.fpsPlayer.doResetMousePosition = not isHidden
        end
    end
    return false
end
...
local function InitGame(game, mapPath)
    ...
    for k, entity in pairs(game.scene.entities) do
        local foundFpsPlayer = entity:GetComponent("FPSPlayer")
        if (foundFpsPlayer ~= nil) then
            game.fpsPlayer = foundFpsPlayer;
            break
        end
    end
    ...
    game.ui = CreateInterface(game.uiCamera, font, frameSize)
    ...
    game.menuPanel = CreatePanel(frameSize.x / 2 - 150, frameSize.y / 2 - 300 / 2, 300, 250, game.ui.background)
